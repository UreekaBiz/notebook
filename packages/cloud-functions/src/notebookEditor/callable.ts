...
async (data, context, userId) => {
  ...
  await updateDocument(userId!/*auth'd*/, data.notebookId, updates, { versionIndex }/*constrain for example*/);
}));
...
async (data, context, userId) => {
  await updateDocument(userId!/*auth'd*/, data.notebookId, [ new InsertText(data.text) ]);
}));
