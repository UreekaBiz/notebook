...
import { generateNodeId, getNodeOutputSpec, isDemo2AsyncNode, AttributeType, ClearNodesDocumentUpdate, CreateCodeBlockNodeDocumentUpdate, Demo2AsyncNodeSpec, NodeName, SetAttributeType, DATA_NODE_TYPE, DEFAULT_DEMO_2_ASYNC_NODE_DELAY, DEFAULT_DEMO_2_ASYNC_NODE_STATUS } from '@ureeka-notebook/web-service';
...
export const Demo2AsyncNode = Node.create<NoOptions, Demo2AsyncNodeStorageType>({
  ...
  addKeyboardShortcuts() {
    return {
      ...
      'Shift-Mod-d': () => applyDocumentUpdates(this.editor, [ new ClearNodesDocumentUpdate(), new CreateCodeBlockNodeDocumentUpdate(NodeName.CODEBLOCK, { [AttributeType.Id]: generateNodeId() })]),
      'Shift-Mod-D': () => applyDocumentUpdates(this.editor, [ new ClearNodesDocumentUpdate(), new CreateCodeBlockNodeDocumentUpdate(NodeName.CODEBLOCK, { [AttributeType.Id]: generateNodeId() })]),
