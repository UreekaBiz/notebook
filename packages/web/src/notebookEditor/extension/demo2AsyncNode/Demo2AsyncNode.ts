...
export const Demo2AsyncNode = Node.create<NoOptions, Demo2AsyncNodeStorageType>({
  ...
  addKeyboardShortcuts() {
    return {
      ...
      'Shift-Mod-d': () => applyDocumentUpdates(this.editor, [ new ClearNodesDocumentUpdate(), new CreateBlockNodeDocumentUpdate(NodeName.CODEBLOCK, { [AttributeType.Id]: generateNodeId() })]),
      'Shift-Mod-D': () => applyDocumentUpdates(this.editor, [ new ClearNodesDocumentUpdate(), new CreateBlockNodeDocumentUpdate(NodeName.CODEBLOCK, { [AttributeType.Id]: generateNodeId() })]),
