...
const dropdownOptions = [
  { value: ListStyle.DECIMAL, label: 'Decimal' },
  { value: ListStyle.LOWER_ALPHA, label: 'Lower Alpha' },
  { value: ListStyle.LOWER_ROMAN, label: 'Lower Roman' },
];
...
export const OrderedListSchemeToolItem: React.FC<EditorToolComponentProps> = ({ editor, depth }) => {
  ...
  const handleChange = (newValue: string) => {
    ...
    // NOTE: focusing the View immediately makes it detect Enter or Tab key presses.
    //       Hence, focus right after the update has been performed
    setTimeout(() => editor.view.focus()/*right after changes*/);
  };

  // == UI ===================================================================
  const value = listItemAtDepth.attrs[AttributeType.ListStyleType] ?? ListStyle.DECIMAL/*default*/;
  return (
