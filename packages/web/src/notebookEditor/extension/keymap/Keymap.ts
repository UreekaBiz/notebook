...
import { isInlineNodeWithContent, isMarkHolderNode, isNestedViewNode, SetNodeSelectionDocumentUpdate } from '@ureeka-notebook/web-service';
...
const selectInlineNodeWithContent = (editor: Editor,  arrowDir: 'up' | 'down') => {
  ...
  try {
    ...
    if(nodeNearby
        ...
        && !isNestedViewNode(nodeNearby/*NVNs should be selected manually*/)
