...
// ********************************************************************************
/**
 * Returns the range that has the word where the editor cursor is currently set
 * @param editor The instance of the editor (its current state)
 * @returns A range with the start and end positions of the word where the cursor is currently set
 */
export const getCurrentWordRange = (editor: Editor): Range => {
  let backwardsSet = false;
  ...
  if(textOffsetPosition === 0) {
    ...
    // if it is at the end, this handling explicitly chooses to do nothing
